...
bootstrap_pagination = register.inclusion_tag(
    'admin/pagination.html')(bootstrap_pagination)
...
class BreadcrumbsNode(template.Node):
    ...
    def render(self, context):
        ...
        try:
            ...
            lines = [l.strip().split(self.delimiter)
                     for l in data.split("\n") if l.strip()]
        else:
        ...
        for d in lines:
            if len(d[0]) > 0:
                ...
                if len(d) == 2:
                    ...
                    raise ValueError(
                        'Invalid breadcrumb line: %s' %
                        self.delimiter.join(d))
        out += '</ul>'
        return out
